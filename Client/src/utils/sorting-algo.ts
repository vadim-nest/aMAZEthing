export function bubbleSort(arr:number[]) {
  ...
  while (hasChanged) {
    ...
    for (let i = 0; i <= arr.length - 2; i++) {
      ...
      if (arr[i] > arr[i + 1]) {
        visitedIndex.push({i, msg:`swapped from ${arr[i]} to ${arr[i+1]}`});
...
export function selectionSort (arr:number[]) {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (min !== i) {
      ...
      visitedIndex.push({msg: `swapped element ${arr[min]} with element ${arr[i]}`});
      ArrayStates.push([...arr]);
    } else {
...
console.log(insertionSort([0, 3, 1, 6, 2, 1]));
